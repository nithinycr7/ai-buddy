  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        {['📘','🧩','🏅','🔬','🧠','✍️'].map((b,i)=> (
          <Card key={i} className="text-center text-3xl">{b}</Card>
        ))}
      </div>
    </div>
  )
    ...
    <div className="space-y-4">
      ...
      <div className="grid md:grid-cols-2 gap-3">
        {[
          ...
          <Card key={i} className="flex items-center gap-2"><span className="text-2xl">{a.icon}</span> <span>{a.text}</span></Card>
        ))}
      </div>
    </div>
  )
