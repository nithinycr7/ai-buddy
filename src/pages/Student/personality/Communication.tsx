  ...
  return (
    <div className="mx-auto max-w-[900px] p-4">
      <div className="flex items-center justify-between">
        ...
        <button className="px-3 py-1.5 rounded-lg border" onClick={reset}>Reset</button>
      </div>

      <div ref={listRef} className="mt-4 h-[55vh] overflow-auto rounded-2xl border bg-white p-3 space-y-2">
        {transcripts.length === 0 && (
          <div className="text-sm text-slate-600">Start by saying something about your day 🙂</div>
        )}
        {transcripts.map(t => (
          <div key={t.id} className={`p-3 rounded-xl ${t.role === 'child' ? 'bg-emerald-50' : 'bg-slate-50'} `}>
            <div className="text-xs text-slate-500 mb-1">{t.role === 'child' ? 'You' : 'Coach'}</div>
            <div>{t.text}</div>
      ...
      <div className="mt-3 flex gap-2">
        ...
        />
        <button className="px-4 py-2 rounded-xl bg-slate-800 text-white" onClick={send}>Send</button>
      </div>
