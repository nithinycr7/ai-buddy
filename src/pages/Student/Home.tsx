    ...
    <div className="space-y-4">
      <div className="flex items-end justify-between gap-3">
        ...
        <div className="text-sm">
          <label htmlFor="class-date" className="mr-2 text-slate-600">Pick a date</label>
          <input
            id="class-date"
            type="date"
            value={date}
          ...
          classesForDay.slice(0,4).map(c => (
            <Card key={c.id}>
              <div className="flex items-center justify-between gap-3">
                <div>
                  <div className="text-lg">{c.icon} {c.title} — {c.topic}</div>
                  <div className="text-sm text-slate-600 max-w-prose">{c.summary}</div>
                  <div className="flex gap-2 mt-2">
                    <a href="/student/replay" className="px-3 py-1.5 rounded-lg bg-pastelGreen">Revise</a>
                    <a href="/student/quiz" className="px-3 py-1.5 rounded-lg bg-pastelYellow">Quiz</a>
                  </div>
                </div>
                <div className="shrink-0"><ProgressRing value={c.progress}/></div>
              </div>
            </Card>
          ))
        )}
