...
const NOTES: Record<SubjectKey, Record<string, TeachingNotes>> = {
  Biology: {
    ...
    Respiration: {
      tips: ['Measure resting vs after-jog breathing rate.', 'Compare aerobic and anaerobic in a quick table.'],
  ...
  Math: {
    'Quadratic Equations': {
      tips: ['Relate solutions to the graph touching/cutting x-axis.', 'Use completing the square visually.'],
  ...
  Chemistry: {
    'Acids & Bases': {
      tips: ['Use pH scale to explain strength.', 'Relate to everyday examples (vinegar, baking soda).'],
...
export const useAppStore = create<AppState>((set, get) => ({
  ...
  // ----- Student
  classes: [
    { id: 'bio', icon: '🌱', title: 'Biology', topic: 'Photosynthesis', progress: 0.65, summary: 'Plants convert light into energy using chlorophyll and CO₂.' },
    { id: 'math', icon: '➗', title: 'Math', topic: 'Quadratics', progress: 0.35, summary: 'Parabolas, vertex form, and solving with factoring or formula.' },
    { id: 'phy', icon: '🧲', title: 'Physics', topic: 'Kinematics', progress: 0.52, summary: 'Motion in 1D/2D, velocity, acceleration, and graphs.' },
    { id: 'his', icon: '📜', title: 'History', topic: 'Ancient Civilizations', progress: 0.8, summary: 'Egypt, Indus Valley, and Mesopotamia foundations.' },
  ],
  leaderboard: [
    { name: 'Aarav', score: 92 },
    { name: 'Diya', score: 89 },
  ...
  upcomingExams: [
    { id: 'ex-bio-01', subject: 'Biology', topic: 'Photosynthesis & Respiration', dateISO: '2025-08-20' },
        ...
        communication: {
          ...
          transcripts: [
            ...s.communication.transcripts,
            {
              ...
              text,
              nudge:
                'Try to add reasons (“because…”) and one follow-up question.',
            },
          ],
        },
      })),
  ...
  getAssignmentsFor: (dateISO: string) =>
    get()._assignments.filter((a) => a.dateISO === dateISO),
  toggleAssignment: (id: string, completed: boolean) =>
    set((s) => ({
      _assignments: s._assignments.map((a) =>
